...
async function settlePayment(c, paymentId) {
  ...
  return withLock('order:'+id,async()=>{
    ...
    // Determine the account environment from the authenticated seller, not
    // solely live_mode: test-account transactions can carry live_mode=true.
    const seller = await mp(c,'/users/me');
    const sellerMatches = String(seller.id) === String(order.sellerId) && seller.site_id === 'MCO';
    const testSeller = Array.isArray(seller.tags) && seller.tags.includes('test_user');
    const modeMatches = sellerMatches && Array.isArray(seller.tags) &&
      (c.production ? !testSeller && payment.live_mode === true : testSeller && typeof payment.live_mode === 'boolean');
    const checks = {
      seller: String(payment.collector_id) === String(order.sellerId),
      currency: payment.currency_id === 'COP',
      amount: Number(payment.transaction_amount) === order.total,
      mode: modeMatches,
      metadata: payment.metadata?.hogarq_order_id === order.id
    };
    const failed = Object.keys(checks).filter(key => !checks[key]);
    if (failed.length) {
      // Log only field names, never payment payloads or customer data.
      console.error('hogarq_payment_validation_failed', failed.join(','));
      throw new HttpError(422,'El pago no corresponde a este pedido.');
    }
