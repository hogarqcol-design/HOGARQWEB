...
let db, mails, preferences, payment, failEmail, sellerAccount;
...
beforeEach(()=>{
 ...
 sellerAccount={id:42,site_id:'MCO',tags:['test_user']};
 ...
 global.fetch=async(url,options={})=>{
  ...
  if(url==='https://redis.example'){
   ...
   data={result};
  }else if(url.endsWith('/users/me')) data=sellerAccount;
...
test('forged signature, wrong amount, currency or seller cannot approve an order',async()=>{
 await call(checkout,'POST','/api/checkout',input());
 assert.equal((await call(webhook,'POST','/api/mercadopago-webhook?data.id=123&type=payment',{})).status,401);
 for(const [key,value] of [['transaction_amount',1],['currency_id','USD'],['collector_id',99]]){
...
test('test-account payments accept either boolean live_mode and receipts remain test-only',async()=>{
 await call(checkout,'POST','/api/checkout',input());
 ...
 assert.equal(mails.length,2);
 assert(mails.every(m=>m.payload.to[0]==='hogarqcol@gmail.com' && m.payload.subject.startsWith('[PRUEBA')));
});
test('test verification rejects real, unknown or mismatched authenticated sellers',async()=>{
 await call(checkout,'POST','/api/checkout',input());
 for(const account of [{id:42,site_id:'MCO',tags:[]},{id:42,site_id:'MCO'},{id:99,site_id:'MCO',tags:['test_user']}]){
  sellerAccount=account;
  assert.equal((await notify()).status,422);
 }
 assert.equal(mails.length,0);
});
test('production validation rejects test accounts and non-live payments',async()=>{
 await call(checkout,'POST','/api/checkout',input());
 const c={...require('../lib/config').config(),production:true};
 payment.live_mode=true;
 await assert.rejects(()=>require('../lib/payments').settlePayment(c,123),{status:422});
 sellerAccount.tags=[];payment.live_mode=false;
 await assert.rejects(()=>require('../lib/payments').settlePayment(c,123),{status:422});
 assert.equal(mails.length,0);
});
